...
export default function UpdatePropertyPage() {
  ...
  const getPropertyData = async () => {
    try {
      ...
      if (res.success) {
        ...
        setToast({ success: false, message: "Failed To Load Property Data" });
      }
    } catch (error) {
      setToast({ success: false, message: "Error Loading Property Data" });
    }
  };
  ...
  const initializeFormData = (propertyData) => {
    ...
    // Parse the existing address into separate fields
    let parsedPlotNumber = propertyData.plotNumber || "";
    let parsedBlock = "";
    let parsedStreetAddress = "";
    let parsedCompleteAddress = propertyData.address || "";

    // Try to parse the existing address (assuming format: "Block, Street, Complete")
    if (propertyData.address) {
      const addressParts = propertyData.address.split(', ');
      if (addressParts.length >= 3) {
        // Check if first part contains plot number indicator
        if (addressParts[0].includes('Plot/House/Office:')) {
          parsedPlotNumber = addressParts[0].replace('Plot/House/Office:', '').trim();
          parsedBlock = addressParts[1] || "";
          parsedStreetAddress = addressParts[2] || "";
          parsedCompleteAddress = addressParts.slice(3).join(', ') || propertyData.address;
        } else {
          parsedBlock = addressParts[0] || "";
          parsedStreetAddress = addressParts[1] || "";
          parsedCompleteAddress = addressParts.slice(2).join(', ') || propertyData.address;
        }
      } else if (addressParts.length === 2) {
        parsedBlock = addressParts[0] || "";
        parsedStreetAddress = addressParts[1] || "";
      } else {
        parsedCompleteAddress = propertyData.address;
      }
    }
    ...
    setAdType(propertyData.adType === "none" ? "normal" : propertyData.adType);
    setPurpose(propertyData.category || "Sell");
    setPaymentPlan(propertyData.paymentPlan || "Monthly");
    setType(propertyData.type || "Houses");
    setSubType(propertyData.subType || "Home");
    setPhase(propertyData.phase || "Phase 1");
    setPlotNumber(parsedPlotNumber);
    setBlock(parsedBlock);
    setStreetAddress(parsedStreetAddress);
    setCompleteAddress(parsedCompleteAddress);
    setPrice(propertyData.price || "");
    setPriceInDollar(propertyData.price ? (parseFloat(propertyData.price) / 282).toFixed(2) : "");
    setArea(propertyData.area || "");
    setAreaUnit(propertyData.areaUnit || "Marla");
    setBedrooms(propertyData.bedrooms || "");
    setBathrooms(propertyData.bathrooms || "");
    setOtherText(propertyData.otherFeatures ? propertyData.otherFeatures.join(", ") : "");

    // Set plot-specific fields
    if (propertyData.plotAmenities) {
      setPlotAmenities(Array.isArray(propertyData.plotAmenities) ? propertyData.plotAmenities : [propertyData.plotAmenities]);
    }
  ...
  const getCurrentAgent = async () => {
    try {
      ...
      setToast({ success: false, message: "Error Loading Agent Data" });
    }
  };
  ...
  const isValid =
    ...
    area.trim() &&
    areaUnit &&
    (featureImageFile || featureImagePreview) &&
    (previewList.length > 0 || imageFiles.length > 0);
  ...
  const addVideo = (e) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      setToast({ success: false, message: "Video Must Be ≤ 50 MB" });
      return;
    }
    setVideoFile(file);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isValid) {
      setToast({ success: false, message: "Please Fill All Required Fields" });
      return;
    }
    ...
    try {
      ...
      // Combine address fields into one string for the backend
      const addressParts = [];
      if (plotNumber) addressParts.push(`Plot/House/Office: ${plotNumber}`);
      if (block) addressParts.push(block);
      if (streetAddress) addressParts.push(streetAddress);
      if (completeAddress) addressParts.push(completeAddress);
      ...
      const res = await propertyService.updateProperty(propertyId, payload, token);
      
      if (res.success) {
        setToast({ success: true, message: res.message || "Property Updated Successfully" });
        // Refresh property data
        getPropertyData();
        // Refresh agent data to update ad counts
        getCurrentAgent();
      } else {
        setToast({ success: false, message: res.message || "Failed To Update Property" });
      }
    } catch (err) {
      setToast({ success: false, message: err.message || "An Error Occurred While Updating Property" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
        <h2 className="text-xl font-bold text-blue-800 mb-2">Update Property</h2>
        <p className="text-blue-600">Editing: {property.title}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <FormBlock heading="Property Media">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Current Images Will Be Replaced If New Ones Are Added</p>
